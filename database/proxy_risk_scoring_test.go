...
func TestGetProxyRiskScoringProfileReleasesSQLiteConnection(t *testing.T) {
	db, err := New("sqlite", filepath.Join(t.TempDir(), "proxy-risk-connection.sqlite"))
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()
	...
	defer cancel()
	id, err := db.CreateProxyRiskScoringProfile(ctx, &ProxyRiskScoringProfile{Name: "single-connection"})
	if err != nil {
		t.Fatal(err)
	}
	for range 3 {
		if _, err := db.GetProxyRiskScoringProfile(ctx, id); err != nil {
			t.Fatalf("read with one available connection: %v", err)
		}
	}
}
