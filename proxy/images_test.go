...
func TestCollectImagesResponseUploadsURLWhenB64NotRequested(t *testing.T) {
	...
	out, _, imageCount, _, err := collectImagesResponse(
		context.Background(),
		strings.NewReader(upstream),
		"url",
		"gpt-image-2",
		func(_ context.Context, image imageCallResult, index int) (string, bool) {
			uploaderCalled = true
			if index != 0 {
			...
			return "https://cdn.example.com/images/generated.png", true
		},
	)
	if err != nil {
		t.Fatalf("collectImagesResponse returned error: %v", err)
